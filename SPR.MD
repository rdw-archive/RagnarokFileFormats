...
* [Palettes](https://en.wikipedia.org/wiki/Palette_(computing)) and [indexed colors](https://en.wikipedia.org/wiki/Indexed_color): By using a predefined lookup table, less space is used to store the pixel data
...
In the later versions, indexed-color pixels that represent the invisible background are compressed using Run-Length Encoding (see above).

This simply means that all "runs" of multiple zero bytes in the indexed-color frames are replaced with ``00 ??`` where ``??`` is the number of zeroes replaced. Decompressing then simply removes ``??`` and replaces it with the respective number of zero bytes again.
...
Likewise, applying this to the RGBA frames would be utterly pointless, considering that the same color already used for transparency would take up four times as much space (four-byte RGBA value vs. one-byte palette index) and be similarly invisible.

## Layout

The file structure differs slightly with the file version, as newer versions added more features. Please consult the tables below for a detailed specification.
...
There are a variety of tools available to parse SPR files and obtain the image in its raw form:
...
* RagnarokSPR (JS): A plugin I wrote to extract the binary data, for educational purposes

After converting the binary data back to the original (bitmap) form, any image editing software can be used to modify them.
