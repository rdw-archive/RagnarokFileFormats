...
* The (much simpler) [RSM](RSM.MD) format description, which also uses keyframe 3D animations
* [Skeletal animation](https://en.wikipedia.org/wiki/Skeletal_animation), a technique for animating 3D models through "bones" and "weights"
...
* [The Wikipedia article on Data Compression](https://en.wikipedia.org/wiki/Data_compression), for an overview of the technical background
* [Huffman Coding](https://en.wikipedia.org/wiki/Huffman_coding) and [LZ77/LZ78](https://en.wikipedia.org/wiki/Lempel-Ziv) form the basis of the Oodle0 compression algorithm

## Overview

GR2 files are used very sparsely in RO, and appear to be a (failed) experiment that took place late in the game's development (in 2003): They are only used to represent WOE Guardians, the Emperium as seen during WOE, castle flags, and treasure boxes. They also contain an unfinished version of what looks to be a 3D player model (skeleton and textures only).

There are two "types" of Granny3D files used in RO: The model and idle pose of each 3D actor and can be found in files located in the ``data/model/3dmob`` directory, while the others store multiple skeletons for the remaining poses and are found in ``data/model/3dmob_bone``.
...
There are ways around this, of course, but they generally rely on specific format conversion (such as a GR2-to-DAE converter that I've seen) or may not be fully compatible with all possible GR2 variations (various open source projects for other games). Reverse-enginering the decompression is another approach that should work, albeit a rather challenging one.

## Features

Granny files can store arbitrary data, but in the case of RO they alway seem to follow the standard granny format. This means only standard Granny3D features are used, though the complexity and closed-source nature of RAD's library makes it difficult to learn how it works.
...
Since Gravity created their files on the same platform the game runs on (Windows/x86), I'm guessing that this feature isn't used, but I haven't researched it further.

This hypothesis is, however, corroborated by the fact we can find the exact signature bytes (``B8 67 B0 CA F8 6D B1 0F 84 72 8C 7E 5E 19 00 1E``) inside ``granny2.dll``, but none of the byte-reversed permutations I tried were present in the version that's shipped with the client.

I'd guess that it might be used for other platforms, but I haven't found any reliable information about it (and I think it doesn't *really* matter as far as this project is concerned).
...
**TBD:** Why is the data split this way? Perhaps to differentiate between persistent and temporary resources (vertex data -> goes to CPU, then can be freed. What parts do you want to keep? Animations and skeletons?)
...
| User Tag | TBD | 4 Bytes | uint32 | Always ``80 00 00 0F`` |
